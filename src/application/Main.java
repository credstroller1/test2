...
public class Main extends Application {
	...
	public static DaemonSupervisor newtestDaemon; 	// Для глобального доступа
	...
	public static void daemonShutDown() throws Exception {
		if(newtestDaemon!=null && newtestDaemon.isAlive()) {
			
			if(newtestDaemon.getOsType()==DaemonSupervisor.OSType.WINDOWS) { // в Windows нет SIGTERM - поэтому через жопу
				
				if(rpcCommander!=null) {
					LOGGER.info(newtestDaemon.getDaemonName()+" shutdown ..."); 
					LOGGER.console(newtestDaemon.getDaemonName()+" shutdown ..."); // TODO debug
					...
					while(newtestDaemon.isAlive() && System.currentTimeMillis()-waitForTime<0);
					
					if(newtestDaemon.isAlive()) { // Жоска дропаем
						LOGGER.warning(newtestDaemon.getDaemonName()+" can't soft stop - forced termination ...");
						LOGGER.console(newtestDaemon.getDaemonName()+" can't soft stop - forced termination ..."); // TODO debug
						newtestDaemon.destroyForcibly();
						if(!newtestDaemon.waitFor(120*1000)) {
							LOGGER.error(newtestDaemon.getDaemonName()+" did not termination ..."); 
							throw new RuntimeException(newtestDaemon.getDaemonName()+" did not termination ...");
						}
					}
					
				}
				else newtestDaemon.shutdown(); // ну тогда остается только жостко дропнуть (не должно быть)
			}
			else newtestDaemon.shutdown();
		}
	}
	public static void daemonStart(String ...additionOptions) throws Exception {
		// FIXME - инвалидация (очистка) всех кешей
		
		if(newtestDaemon!=null) newtestDaemon.start(additionOptions);
	}
	...
	@Override public void init() throws Exception { LOGGER.console("𝕝𝕠𝕟𝕘𝕔𝕠𝕚𝕟-𝕛𝕒𝕧𝕒");
		...
		(newtestDaemon = new DaemonSupervisor()).start(); // Помнить, что демон стартует какое-то время и запросы по RPC начинает принимать не сразу
		rpcCommander = new RPCCommanderCached( newtestDaemon.getPort(),newtestDaemon.getUser(),newtestDaemon.getPassword() );
	...
	@Override public void start(Stage primaryStage) {
		try {
			...
			primaryStage.getIcons().add(new Image("res/newtest_128x128.png"));
	...
	@Override public void stop() throws Exception {
		...
		LOGGER.info("newtest-java Application exit");
		LOGGER.console("newtest-java Application exit");		
	}
	...
	void menuInit() {
	    ...
	    toolsMenu.setOnShowing((ev)->{ // Профилактика кошелька возможна только если мы управляем демоном
	    	menuItemWalletPrevention.setDisable(!newtestDaemon.isAlive());
	    });   
		...
		statusController.addListener((obj)-> { if(obj instanceof Listener.FullStatus) { // состояние меню шифровки кошелька зависит от состояния кошелька
			...
			Platform.runLater(()->{
				if(status.unlocked_until>=0) { // Кошелек уже зашифрован - можно только менять пароль
					...
					menuItemEncryptWallet.setDisable(!newtestDaemon.isAlive());
					menuItemUnlockWallet.setDisable(true);
					menuItemChangePassword.setDisable(true);					
				}
			});
